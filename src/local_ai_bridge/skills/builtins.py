...
def register_builtin_skills(registry: SkillRegistry) -> None:
    registry.register(
        SkillSpec(
            ...
            lambda ctx, p: SkillResult(
                True,
                "Report generato.",
                build_super_report(_workspace(ctx), p.get("task", "")),
            ),
        )
    )
    registry.register(
    ...
    registry.register(
        SkillSpec(
            ...
            (
                "Unisce tutti i messaggi dei commit Git in una cronologia "
                "Markdown persistente."
            ),
            frozenset({"workspace.read", "git.read"}),
            lambda ctx, p: SkillResult(
                True,
                "Changelog Git generato.",
                commit_history_markdown(_workspace(ctx)),
            ),
        )
    )
