  ...
  <div class="details-body">
    ...
    <div class="power-user-grid">
      <div class="settings-panel">
        ...
        <p class="field-help"><strong>Superpoteri Markdown:</strong> i file Markdown vengono salvati nella cartella dati di BridgAI e condivisi tra tutti i progetti. Richiamali nel task con <code>@superpower:id</code>.</p>
      </div>
      <div class="settings-panel">
        ...
        <select id="preferredWebAi" onchange="applyPreferredWebAiPreset()"><option value="chatgpt">ChatGPT</option><option value="claude">Claude</option><option value="gemini">Gemini</option><option value="deepseek">DeepSeek</option><option value="custom">Personalizzato</option></select>
        <p class="field-help">ChatGPT e Claude usano ZIP → ZIP; Gemini usa ZIP → File Markdown di aggiornamento; DeepSeek usa Markdown → File Markdown di aggiornamento. Con Personalizzato scegli il flusso.</p>
            ...
            <div class="compatibility-table-wrap">
              <table class="compatibility-table">
                ...
                <tbody>
                  ...
                  <tr><th scope="row">DeepSeek</th><td>Markdown</td><td>Markdown</td></tr>
...
<div class="simple-request-toolbar">
  <div class="superpower-actions" aria-label="Superpoteri e note">
    <button class="secondary icon-button" type="button" onclick="openSuperpowers()"><svg class="button-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m13 2-2 8h7l-9 12 2-8H4z"/></svg><span>Richiama superpoteri</span></button>
    <button class="secondary icon-button" type="button" onclick="openProjectNotes()"><svg class="button-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M4 4h16v16H4z"/><path d="M8 9h8M8 13h8M8 17h5"/></svg><span>Note e attività</span></button>
    <span id="superpowerSelectionSummary" class="muted">Nessun superpotere selezionato</span>
  </div>
  <div class="request-actions"><button onclick="generateReport()">Prepara richiesta per l’AI</button><button id="automationSendButton" class="success" onclick="sendReportToAutomation()" disabled>Invia automaticamente a ChatGPT</button></div>
  <div class="provider-actions">
    ...
    <button data-provider="deepseek" disabled onclick="openProvider('https://chat.deepseek.com/')">Continua su DeepSeek</button>
  </div>
</div>
...
<div id="preview" class="preview">
  ...
  <h3>Recap aggiornamento</h3><pre id="updateRecap" class="update-recap"></pre>
...
<div id="superpowerModal" class="modal" hidden><div class="modal-panel"><div class="modal-header"><h2>Superpoteri Markdown</h2><button class="secondary" type="button" onclick="closeSuperpowers()">Chiudi</button></div><p class="muted">Seleziona i superpoteri da richiamare oppure creali e modificali. La libreria è condivisa tra tutti i progetti.</p><div class="superpower-grid"><div><label for="superpowerSearch">Cerca</label><input id="superpowerSearch" type="search" placeholder="Cerca per nome, ID o descrizione…" oninput="renderSuperpowers()"><label for="superpowerCategoryFilter">Categoria</label><select id="superpowerCategoryFilter" onchange="renderSuperpowers()"><option value="">Tutte le categorie</option></select><div id="superpowerList" class="superpower-list"></div></div><div><label for="superpowerTitle">Nome</label><input id="superpowerTitle"><label for="superpowerId">ID</label><input id="superpowerId"><label for="superpowerDescription">Descrizione</label><input id="superpowerDescription"><label for="superpowerCategory">Categoria</label><input id="superpowerCategory" value="Generale"><label for="superpowerMarkdown">Markdown</label><textarea id="superpowerMarkdown"></textarea><div class="actions"><button type="button" onclick="saveSuperpower()">Salva</button><button class="danger" type="button" onclick="deleteSuperpower()">Elimina</button><button class="secondary" type="button" onclick="newSuperpower()">Nuovo</button></div><div id="superpowerResult" class="feedback"></div></div></div><div class="actions"><button class="success" type="button" onclick="applySuperpowerSelection()">Usa i selezionati</button></div></div></div>
<div id="projectNotesModal" class="modal notes-modal" hidden><div class="modal-panel"><div class="modal-header"><div><p class="eyebrow">Workspace</p><h2>Note e attività</h2><p class="muted">Salva idee e todo locali al progetto corrente e aggiungili alla richiesta AI con un clic.</p></div><button class="secondary" type="button" onclick="closeProjectNotes()">Chiudi</button></div><div class="notes-grid"><aside class="notes-sidebar"><div class="notes-toolbar"><input id="projectNoteSearch" type="search" placeholder="Cerca nelle note…" oninput="renderProjectNotes()"><button class="secondary" type="button" onclick="newProjectNote()">+ Nuova</button></div><p id="projectNotesSummary" class="muted">Nessuna nota caricata.</p><div id="projectNotesList" class="superpower-list notes-list"></div></aside><section class="notes-editor"><label for="projectNoteTitle">Titolo</label><input id="projectNoteTitle" placeholder="Es. decisione, bug da correggere, promemoria…"><div class="note-checks"><label class="switch-row" for="projectNoteTodo"><input id="projectNoteTodo" type="checkbox"><span class="switch-track" aria-hidden="true"></span><span>Attività da completare</span></label><label class="switch-row" for="projectNoteCompleted"><input id="projectNoteCompleted" type="checkbox"><span class="switch-track" aria-hidden="true"></span><span>Completata</span></label></div><label for="projectNoteContent">Dettagli</label><textarea id="projectNoteContent" placeholder="Scrivi il contesto utile per te o per l’AI…"></textarea><div class="actions note-actions"><button type="button" onclick="saveProjectNote()">Salva</button><button class="success" type="button" onclick="addProjectNoteToTask()">Aggiungi alla richiesta</button><button class="danger" type="button" onclick="deleteProjectNote()">Elimina</button></div><div id="projectNotesResult" class="feedback"></div></section></div></div></div>
<div id="batchReportModal" class="modal" hidden><div class="modal-panel batch-report-panel"><div class="modal-header"><div><p class="eyebrow">Report batch</p><h2>Creazione report progetti</h2><p id="batchReportSummary" class="muted">Preparazione…</p></div><button id="batchReportCloseButton" class="secondary" type="button" onclick="closeBatchReportModal()" disabled>Chiudi</button></div><ol id="batchReportSteps" class="batch-flow-list"></ol><div id="batchReportModalResult" class="feedback"></div></div></div>
